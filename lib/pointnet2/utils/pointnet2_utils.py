...
class MatrixKMinimums(Function):
    """docstring for MatrixKMinimums"""
    @staticmethod
    def forward(
        ctx, radius: float, nsample: int, matrix: torch.Tensor
        )->torch.Tensor:
        assert matrix.is_contiguous()
        B, N, M = matrix.size()
        if matrix.is_cuda:
            idx = matrix.new(B, N, nsample).zero_().int()
            # idx = torch.zeros(B, N, nsample).int().cuda().to(matrix.device)
            # print(idx)
            pointnet2.matrix_k_minimums_wrapper(
                B, N, M, radius, nsample, matrix, idx
            )
        else:
            idx = torch.IntTensor(B, N, nsample).zero_()
            pointnet2.matrix_k_minimums_cpu(B, N, M, radius, nsample, matrix, idx)
...
matrix_k_min = MatrixKMinimums.apply
