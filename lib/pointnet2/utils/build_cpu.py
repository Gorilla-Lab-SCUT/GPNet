...
def parse_args():
    ...
    clean_arg.add_argument("--build", default=True, dest='build', action="store_true")
    clean_arg.add_argument("--clean", dest='clean', action="store_true")
    parser.set_defaults(build=True, clean=False)
...
def build(args):
    ...
    ffi = create_extension(
        '_ext.pointnet2',
        headers=[a for a in glob.glob("cinclude/matrix_k_minimums_cpu_wrapper.h")],
        sources=[a for a in glob.glob("csrc/matrix_k_minimums_cpu.c")],
